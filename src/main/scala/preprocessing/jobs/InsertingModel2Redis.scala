  ...
  def main(args: Array[String]): Unit = {
    ...
    reviewDf.foreachPartition((rows: Iterator[Review]) => {
      ...
      rows.foreach(review => {
        ...
        cluster.hmset(s"business:${review.business_id}:review:${review.review_id}", review.toMap.asJava)

        cluster.hset(s"user:${review.user_id}:reviews", review.review_id, review.business_id)
      })
