...
object InsertingModel2Memcached {
  def main(args: Array[String]): Unit = {
    ...
    val sparkSession = SparkSession.builder
      .appName("Inserting Model2 Memcached App")
    ...
    reviewDf.foreachPartition((rows: Iterator[Review]) => {
      ...
      rows.foreach(review => {
        ...
        cluster.set(s"business:${review.business_id}:review:${review.review_id}", serialize(review.allToMap), Duration.Inf)

        client.append(s"user:${review.user_id}:reviews", s"${review.review_id}-${review.business_id}:")
      })
